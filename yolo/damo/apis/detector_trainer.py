...
class Trainer:
    def __init__(self, cfg, args, tea_cfg=None, is_train=True):
        ...
        if cfg.train.freeze_bn_name_pattern is not None:
            for name, m in self.model.named_modules():
                if isinstance(m, NORMS) and re.match(cfg.train.freeze_bn_name_pattern, name) is not None:
                    m.track_running_stats = False
                    logger.info('Turn off track_running_stats for: {}'.format(name))
        if cfg.train.freeze_param_name_pattern is not None:
            for name, param in self.model.named_parameters():
                if re.match(cfg.train.freeze_param_name_pattern, name) is not None:
                    logger.info('Turn off requires_grad for: {}'.format(name))
        ...
        if self.cfg.train.finetune_path is not None:
            self.model.load_pretrain_detector(self.cfg.train.finetune_path, self.cfg.train.load_head_weights)
            self.epoch = 0
            self.start_epoch = 0
    ...
    def setup_iters(self, iters_per_epoch, start_epoch, total_epochs,
        ...
        self.iters_per_epoch = iters_per_epoch
        self.start_iter = int(start_epoch * iters_per_epoch)
        self.total_iters = int(total_epochs * iters_per_epoch)
        self.warmup_iters = int(warmup_epochs * iters_per_epoch)
        self.no_aug_iters = int(no_aug_epochs * iters_per_epoch)
        self.no_aug = self.start_iter >= self.total_iters - self.no_aug_iters
        self.eval_interval_iters = int(eval_interval_epochs * iters_per_epoch)
        self.ckpt_interval_iters = int(ckpt_interval_epochs * iters_per_epoch)
    ...
    def build_optimizer(self, models, cfg, exp_module=None):
        ...
        for p, pconfig in param_dict.items():
            logger.info('Train: {}'.format(pconfig["name"]))
