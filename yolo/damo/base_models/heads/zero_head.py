...
from ..core.utils import Scale, Conv2dWithScale, Conv2dCos, Conv2dFreezed, FilterBackward, multi_apply, reduce_mean
...
class ZeroHead(nn.Module):
    ...
    def __init__(
            ...
            n_vectors_for_contour=None,
            use_preclass_BN=False,
            use_coslogit=False,
            use_freeze_conv=False,
            cls_kernel_size=None,
            **kwargs):
        self.n_vectors_for_contour = n_vectors_for_contour
        self.use_preclass_BN = use_preclass_BN
        self.use_coslogit = use_coslogit
        self.use_freeze_conv = use_freeze_conv
        self.cls_kernel_size = cls_kernel_size if cls_kernel_size is not None else last_kernel_size
    ...
    def _init_layers(self):
        ...
        if self.use_preclass_BN:
            for i, nc in enumerate(self.feat_channels):
                self.cls_convs[i].append(nn.BatchNorm2d(nc, affine=False))


        assert (self.n_vectors_for_contour is not None) + self.use_coslogit + self.use_freeze_conv <= 1
        if self.n_vectors_for_contour is not None:
            self.gfl_cls = nn.ModuleList([
                Conv2dWithScale(self.n_vectors_for_contour, self.feat_channels[i],
                        self.cls_out_channels,
                        self.cls_kernel_size, # 3
                        padding=self.cls_kernel_size//2) for i in range(len(self.strides))
            ])
        elif self.use_coslogit:
            self.gfl_cls = nn.ModuleList([
                Conv2dCos(self.feat_channels[i],
                        self.cls_out_channels,
                        self.cls_kernel_size, # 3
                        padding=self.cls_kernel_size//2) for i in range(len(self.strides))
            ])
        elif self.use_freeze_conv:
            self.gfl_cls = nn.ModuleList([
                Conv2dFreezed(self.feat_channels[i],
                        self.cls_out_channels,
                        self.cls_kernel_size, # 3
                        padding=self.cls_kernel_size//2) for i in range(len(self.strides))
            ])
        ...
        self.filter_backward = FilterBackward.apply
    ...
    def forward_train(self, xin, labels=None, imgs=None, aux_targets=None):
        ...
        cls_scores = self.filter_backward(torch.cat(cls_scores, dim=1))
        bbox_preds = self.filter_backward(torch.cat(bbox_preds, dim=1))
        bbox_before_softmax = self.filter_backward(torch.cat(bbox_before_softmax, dim=1))
