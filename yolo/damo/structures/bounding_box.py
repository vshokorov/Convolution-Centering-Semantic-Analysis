...
class Segmentation_extra_field():
    ...
    def resize(self, size, *args, **kwargs):
        ...
        scaled_contours = [c * ratios for c in self.contours]
        ef = Segmentation_extra_field(scaled_contours, size)
        return ef
    ...
    def transpose(self, method):
        ...
        def transpose_one(contour):
            new_c = contour.copy()
            if method == FLIP_LEFT_RIGHT:
                new_c[:, 0] = image_width - new_c[:, 0]
            elif method == FLIP_TOP_BOTTOM:
                new_c[:, 1] = image_height - new_c[:, 1]
            return new_c

        transposed_contours = [transpose_one(c) for c in self.contours]
        ef = Segmentation_extra_field(transposed_contours, self.size)
        return ef

    def crop(self, box):
        w, h = box[2] - box[0], box[3] - box[1]
        
        def crop_one(contour):
            new_c = contour.copy()
            new_c[:, 0] = (new_c[:, 0] - box[0]).clip(min=0, max=w)
            new_c[:, 1] = (new_c[:, 1] - box[1]).clip(min=0, max=h)
            return new_c

        croped_contours = [crop_one(c) for c in self.contours]
        ef = Segmentation_extra_field(croped_contours, (w, h))
        return ef