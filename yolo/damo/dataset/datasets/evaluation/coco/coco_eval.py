...
def do_coco_evaluation(
    ...
    for iou_type in iou_types:
        with tempfile.NamedTemporaryFile() as f:
            ...
            results.update(res)
            res_stor[iou_type] = {
                'iouThrs':      res.eval['params'].iouThrs,     # [.5:.05:.95] T=10 IoU thresholds for evaluation
                'recThrs':      res.eval['params'].recThrs,     # [0:.01:1] R=101 recall thresholds for evaluation
                'catIds':       res.eval['params'].catIds,      # [all] K cat ids to use for evaluation
                'areaRng':      res.eval['params'].areaRng,     # [...] A=4 object area ranges for evaluation
                'areaRngLbl':   res.eval['params'].areaRngLbl, 
                'maxDets':      res.eval['params'].maxDets,     # [1 10 100] M=3 thresholds on max detections per image

                'precision':    res.eval['precision'],          # [TxRxKxAxM] precision for every evaluation setting
                'recall':       res.eval['recall'],             # [TxKxAxM] max recall for every evaluation setting
            }
            logger.info(f"iou_type: {iou_type}. {res.eval['precision'][0, :, :, 0, 2].mean(1)}") 
    ...
    check_expected_results(results, expected_results,
                           expected_results_sigma_tol)
    if output_folder:
        torch.save({'results': results, 'res_stor': res_stor}, os.path.join(output_folder, 'coco_results.pth'))
